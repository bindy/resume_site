...
var SiteJc = (function(){
	...
	function init(){
		...
		$body.on('keydown',function(e){
			...
			var keyLength = 80;
			if($section.css('-webkit-transform') == 'translateY(-828px)'){
...
function getWorkData(workWrap){
	...
	$.ajax({
		url:'http://love-swap.org/site_jc/workdata.json',
		dataType : 'json',
		success : function(data){
			...
			for(var i in data){
				for(var j in data[i]){
					img.src = 'http://love-swap.org/site_jc/'+data[i][j].src;
				}
...
function getPhotoData(workWrap){
	...
	$.ajax({
		url:'http://love-swap.org/site_jc/photodata.json',
		dataType : 'json',
		success : function(data){
			var workColumnNum = Object.getOwnPropertyNames(data).length;
			var img = new Image();
